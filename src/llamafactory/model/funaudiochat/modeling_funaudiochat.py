...
class FunAudioChatForConditionalGeneration(FunAudioChatPreTrainedModel, GenerationMixin):
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # Hugging Face Trainer/Seq2SeqTrainer expects `generate()` to return a tensor of token ids.
        # FunAudioChat can optionally generate speech tokens as a second output, but for S2T-only
        # (`disable_speech=True`) we return text ids only to remain HF-compatible (e.g. predict_with_generate).
        if self.sp_gen_kwargs.get("disable_speech", False):
            return input_ids
