...
class FunAudioChatProcessor(ProcessorMixin):
    ...
    def _regularize_audios(
        ...
        if librosa is None and any(isinstance(audio, (str, BinaryIO)) for audio in audios):
            raise ImportError(
                "Loading FunAudioChat audio inputs from paths/streams requires `librosa`. "
        ...
        if audio is not None:
            ...
            if is_wav_list:
                ...
                if output_kwargs["audio_kwargs"].get("sampling_rate", None) is not None:
                    if librosa is None:
                        raise ImportError(
                            "Resampling FunAudioChat audio inputs requires `librosa`. "
                            "Please install it, e.g. `pip install librosa`."
                        )
